...
static const MSGUID parent_vhdx_guid __attribute__((unused)) = {.data1 = 0xb04aefb7,
                                                                .data2 = 0xd19e,
                                                                .data3 = 0x4a81,
...
vhdx_parse_metadata(BlockDriverState *bs, BDRVVHDXState *s)
{
    ...
    ret = bdrv_pread(bs->file,
                     s->metadata_entries.file_parameters_entry.offset + s->metadata_rt.file_offset,
                     sizeof(s->params),
                     &s->params,
                     0);
    ...
    ret = bdrv_pread(bs->file,
                     s->metadata_entries.virtual_disk_size_entry.offset + s->metadata_rt.file_offset,
                     sizeof(uint64_t),
                     &s->virtual_disk_size,
                     0);
    if (ret < 0)
    ...
    ret = bdrv_pread(bs->file,
                     s->metadata_entries.logical_sector_size_entry.offset + s->metadata_rt.file_offset,
                     sizeof(uint32_t),
    ...
    ret = bdrv_pread(bs->file,
                     s->metadata_entries.phys_sector_size_entry.offset + s->metadata_rt.file_offset,
                     sizeof(uint32_t),
                ...
                uint32_t length, Error **errp)
{
        ...
        has_zero_init == 0)
    {
        ...
        block_state = type == VHDX_TYPE_FIXED ? PAYLOAD_BLOCK_FULLY_PRESENT : PAYLOAD_BLOCK_NOT_PRESENT;
