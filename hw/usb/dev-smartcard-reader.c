...
#define CCID_PRODUCT_DESCRIPTION "SCR3310 USB Smart Card Reader"
#define CCID_VENDOR_DESCRIPTION "SCM Microsystems"
#define CCID_INTERFACE_NAME "USB Smart Card Reader"
#define CCID_SERIAL_NUMBER_STRING "SCM12345678"
...
static uint8_t ccid_card_status(USBCCIDState *s)
{
    return ccid_card_inserted(s)
               ? (s->powered ? ICC_STATUS_PRESENT_ACTIVE
                             : ICC_STATUS_PRESENT_INACTIVE)
               : ICC_STATUS_NOT_PRESENT;
}
                                       ...
                                       unsigned int max_packet_size)
{
    ...
    if (s->current_bulk_in != NULL)
    {
        ...
        if (len)
        {
            usb_packet_copy(p, s->current_bulk_in->data + s->current_bulk_in->pos, len);
        }
        s->current_bulk_in->pos += len;
        if (s->current_bulk_in->pos == s->current_bulk_in->len && len != max_packet_size)
        {
