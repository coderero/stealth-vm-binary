...
struct MTPObject
{
    ...
    MTPObject *parent;
    uint32_t nchildren;
    QLIST_HEAD(, MTPObject)
    children;
    QLIST_ENTRY(MTPObject)
    list;
    bool have_children;
    QTAILQ_ENTRY(MTPObject)
    next;
};
...
struct MTPState
{
    ...
    QTAILQ_HEAD(, MTPObject)
    objects;
    QFileMonitor *file_monitor;
    QTAILQ_HEAD(, MTPMonEntry)
    events;
...
#define MTP_MANUFACTURER "Samsung"
#define MTP_PRODUCT "Galaxy A52"
...
static void usb_mtp_write_data(MTPState *s, uint32_t handle)
{
    ...
    case WRITE_START:
        ...
        if (s->dataset.filename)
        {
            ...
            d->fd = open(path, O_CREAT | O_WRONLY | O_CLOEXEC | O_NOFOLLOW, mask & 0666);
            if (d->fd == -1)
            {
                ret = 1;
                goto done;
...
static void usb_mtp_handle_data(USBDevice *dev, USBPacket *p)
{
    ...
    case EP_DATA_OUT:
        ...
        case TYPE_COMMAND:
            ...
            cmd.argc = (le32_to_cpu(container.length) - sizeof(container)) / sizeof(uint32_t);
