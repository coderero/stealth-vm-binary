...
#define NVME_GUEST_ERR(trace, fmt, ...)                                                      \
    ...
    {                                                                                        \
        ...
        qemu_log_mask(LOG_GUEST_ERROR, #trace " in %s: " fmt "\n", __func__, ##__VA_ARGS__); \
    } while (0)
...
{
    size_t entry_siz = sizeof(NvmeFdpDescrHdr) + nruh * sizeof(NvmeRuhDescr) + vss;
...
static void nvme_process_sq(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq) || QTAILQ_EMPTY(&sq->req_list)))
    {
        ...
        status = sq->sqid ? nvme_io_cmd(n, req) : nvme_admin_cmd(n, req);
    ...
    if (params->sriov_max_vfs)
    {
        ...
        if (params->sriov_vq_flexible < params->sriov_max_vfs * 2)
        {
            error_setg(errp, "sriov_vq_flexible must be greater than or equal"
                             " to %d (sriov_max_vfs * 2)",
                       params->sriov_max_vfs * 2);
            return false;
        }
...
static void nvme_init_state(NvmeCtrl *n)
{
    ...
    else
    {
        ...
        cap->vqfrsm = n->params.sriov_max_vq_per_vf ? cpu_to_le16(n->params.sriov_max_vq_per_vf) : cap->vqfrt / MAX(max_vfs, 1);
    }
    ...
    else
    {
        ...
        cap->vifrsm = n->params.sriov_max_vi_per_vf ? cpu_to_le16(n->params.sriov_max_vi_per_vf) : cap->vifrt / MAX(max_vfs, 1);
    }
...
static bool nvme_init_sriov(NvmeCtrl *n, PCIDevice *pci_dev, uint16_t offset,
                            Error **errp)
{
    uint16_t vf_dev_id = n->params.use_intel_id ? PCI_DEVICE_ID_INTEL_NVME : PCI_DEVICE_ID_REDHAT_NVME;
    NvmePriCtrlCap *cap = &n->pri_ctrl_cap;
    ...
    pcie_sriov_pf_init_vf_bar(pci_dev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY | PCI_BASE_ADDRESS_MEM_TYPE_64, bar_size);
...
static bool nvme_init_pci(NvmeCtrl *n, PCIDevice *pci_dev, Error **errp)
{
    ...
    if (n->params.msix_exclusive_bar && !pci_is_vf(pci_dev))
    {
        ...
        memory_region_init_io(&n->iomem, OBJECT(n), &nvme_mmio_ops, n, "nvme",
                              bar_size);
        pci_register_bar(pci_dev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY | PCI_BASE_ADDRESS_MEM_TYPE_64, &n->iomem);
    ...
    else
    {
        ...
        {
            pci_register_bar(pci_dev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY | PCI_BASE_ADDRESS_MEM_TYPE_64, &n->bar0);
        }
    ...
    {
        uint16_t doe_offset = n->params.sriov_max_vfs ? PCI_CONFIG_SPACE_SIZE + PCI_ARI_SIZEOF
...
static void nvme_init_ctrl(NvmeCtrl *n, PCIDevice *pci_dev)
{
    ...
    strpadcpy((char *)id->mn, sizeof(id->mn), "Samsung SSD 980 PRO", ' ');
                                   ...
                                   void *opaque, Error **errp)
{
    ...
    cap = NVME_SMART_SPARE | NVME_SMART_TEMPERATURE | NVME_SMART_RELIABILITY | NVME_SMART_MEDIA_READ_ONLY | NVME_SMART_FAILED_VOLATILE_MEDIA;
