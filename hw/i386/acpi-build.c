                                  ...
                                  AcpiFadtData *data)
{
     ...
     */
    bool smm_enabled = object_property_get_bool(o, "smm-compat", NULL) ? true : x86_machine_is_smm_enabled(x86ms);
    ...
    AcpiFadtData fadt = {
        ...
        .flags =
             ...
             * used
             */
            ((ms->smp.max_cpus > 8) ? (1 << ACPI_FADT_F_FORCE_APIC_CLUSTER_MODEL) : 0),
        ...
        .pm1a_cnt = {.space_id = as, .bit_width = 2 * 8, .address = io + 0x04},
        .pm_tmr = {.space_id = as, .bit_width = 4 * 8, .address = io + 0x08},
        .gpe0_blk = {.space_id = as, .bit_width = object_property_get_uint(o, ACPI_PM_PROP_GPE0_BLK_LEN, NULL) * 8, .address = object_property_get_uint(o, ACPI_PM_PROP_GPE0_BLK, NULL)},
    };
           ...
           Range *pci_hole, Range *pci_hole64, MachineState *machine)
{
    ...
    AcpiTable table = {.sig = "DSDT", .rev = 1, .oem_id = x86ms->oem_id, .oem_table_id = x86ms->oem_table_id};
...
{
    AcpiTable table = {.sig = "HPET", .rev = 1, .oem_id = oem_id, .oem_table_id = oem_table_id};
               ...
               const char *oem_id, const char *oem_table_id)
{
    ...
    AcpiTable table = {.sig = "TCPA", .rev = 2, .oem_id = oem_id, .oem_table_id = oem_table_id};
...
build_srat(GArray *table_data, BIOSLinker *linker, MachineState *machine)
{
    ...
    AcpiTable table = {.sig = "SRAT", .rev = 1, .oem_id = x86ms->oem_id, .oem_table_id = x86ms->oem_table_id};
               ...
               const char *oem_table_id)
{
    ...
    AcpiTable table = {.sig = "DMAR", .rev = 1, .oem_id = oem_id, .oem_table_id = oem_table_id};
...
build_waet(GArray *table_data, BIOSLinker *linker, const char *oem_id,
           const char *oem_table_id)
{
    AcpiTable table = {.sig = "WAET", .rev = 1, .oem_id = oem_id, .oem_table_id = oem_table_id};
                ...
                const char *oem_table_id)
{
    ...
    AcpiTable table = {.sig = "IVRS", .rev = 1, .oem_id = oem_id, .oem_table_id = oem_table_id};
