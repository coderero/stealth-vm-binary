                               ...
                               uint32_t *msi_pcie_phandle)
{
    ...
    qemu_fdt_setprop_cell(ms->fdt, "/cpus", "timebase-frequency",
                          kvm_enabled() ? kvm_riscv_get_timebase_frequency(&s->soc->harts[0]) : RISCV_ACLINT_DEFAULT_TIMEBASE_FREQ);
...
static void virt_build_smbios(RISCVVirtState *s)
{
    ...
    const char *product = "ASUS NovaGo TP370QL";

    if (kvm_enabled())
    {
        product = "ASUS NovaGo TP370QL";
    }

    smbios_set_defaults("ASUSTeK COMPUTER INC.", "NovaGo TP370QL", mc->name);
