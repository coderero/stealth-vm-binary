...
static int scsi_disk_emulate_mode_sense(SCSIDiskReq *r, uint8_t *outbuf)
{
    ...
    trace_scsi_disk_emulate_mode_sense((r->req.cmd.buf[0] == MODE_SENSE) ? 6 : 10, page, r->req.cmd.xfer, page_control);
...
static void scsi_hd_realize(SCSIDevice *dev, Error **errp)
{
    ...
    if (!s->product)
    {
        s->product = g_strdup("Samsung SSD 860 EVO");
    }
