 ...
 * support it).
 */
int build_append_named_dword(GArray *array, const char *name_format, ...)
{
    int offset;
    va_list ap;
                   ...
                   uint32_t *irq_list, uint8_t irq_count)
{
    int i;
    Aml *var = aml_alloc();
    uint8_t irq_flags = con_and_pro | (level_and_edge << 1) | (high_and_low << 2) | (shared << 3);
...
static Aml *create_field_common(int opcode, Aml *srcbuf, Aml *index, const char *name)
{
 ...
 */
void build_rsdp(GArray *tbl, BIOSLinker *linker, AcpiRsdpData *rsdp_data)
{
 ...
 */
void build_rsdt(GArray *table_data, BIOSLinker *linker, GArray *table_offsets,
                const char *oem_id, const char *oem_table_id)
{
    int i;
    AcpiTable table = {.sig = "RSDT", .rev = 1, .oem_id = oem_id, .oem_table_id = oem_table_id};
...
{
    int i;
    AcpiTable table = {.sig = "XSDT", .rev = 1, .oem_id = oem_id, .oem_table_id = oem_table_id};
                ...
                const char *oem_id, const char *oem_table_id)
{
    ...
    AcpiTable table = {.sig = "SLIT", .rev = 1, .oem_id = oem_id, .oem_table_id = oem_table_id};
...
{
    AcpiTable table = {.sig = "SPCR", .rev = rev, .oem_id = oem_id, .oem_table_id = oem_table_id};
                ...
                const char *oem_id, const char *oem_table_id)
{
    ...
    int n;
    AcpiTable table = {.sig = "PPTT", .rev = 2, .oem_id = oem_id, .oem_table_id = oem_table_id};
...
{
    int off;
    AcpiTable table = {.sig = "FACP", .rev = f->rev, .oem_id = oem_id, .oem_table_id = oem_table_id};
                ...
                const char *oem_id, const char *oem_table_id)
{
    ...
    AcpiTable table = {.sig = "TPM2", .rev = 4, .oem_id = oem_id, .oem_table_id = oem_table_id};
