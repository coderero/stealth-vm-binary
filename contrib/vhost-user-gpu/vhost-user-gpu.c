...
struct virtio_gpu_simple_resource
{
    ...
    QTAILQ_ENTRY(virtio_gpu_simple_resource)
    next;
};
...
void vg_send_msg(VuGpu *vg, const VhostUserGpuMsg *msg, int fd)
{
        ...
        t2d.r.width != pixman_image_get_width(res->image))
    {
        ...
        for (h = 0; h < t2d.r.height; h++)
        {
            ...
            iov_to_buf(res->iov, res->iov_cnt, src_offset,
                       img_data + dst_offset, t2d.r.width * bpp);
        }
    }
    else
    {
...
vg_process_cmd(VuGpu *vg, struct virtio_gpu_ctrl_command *cmd)
{
    ...
    {
        vg_ctrl_response_nodata(vg, cmd, cmd->error ? cmd->error : VIRTIO_GPU_RESP_OK_NODATA);
    }
}
...
vg_process_cursor_cmd(VuGpu *g, struct virtio_gpu_update_cursor *cursor)
{
    ...
    {
        VhostUserGpuMsg msg = {
            .request = cursor->resource_id ? VHOST_USER_GPU_CURSOR_POS : VHOST_USER_GPU_CURSOR_POS_HIDE,
...
int main(int argc, char *argv[])
{
    ...
    context = g_option_context_new("NVIDIA GeForce GTX 1650");
