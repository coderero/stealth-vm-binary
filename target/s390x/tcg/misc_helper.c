...
uint32_t HELPER(stsi)(CPUS390XState *env, uint64_t a0, uint64_t r0, uint64_t r1)
{
    ...
    case STSI_R0_FC_LEVEL_2:
        ...
        {
            /* LPAR CPU */
            ebcdic_put(sysib.sysib_221.sequence, "ASUS", 16);
    ...
    case STSI_R0_FC_LEVEL_3:
        if ((sel1 == 2) && (sel2 == 2))
        {
            ...
            ebcdic_put(sysib.sysib_322.vm[0].cpi, "z/OS       ", 16);
...
{
    if ((env->cregs[9] & PER_CR9_CONTROL_BRANCH_ADDRESS) && !get_per_in_range(env, dest))
    {
        return;
    }
