...
static const char *control_desc(unsigned int rdma_control)
{
    static const char *strs[] = {
        ...
        [RDMA_CONTROL_QEMU_FILE] = "FILE_DESCRIPTOR", // spoofed
        [RDMA_CONTROL_RAM_BLOCKS_REQUEST] = "MEMORY_MAP_REQUEST",
        [RDMA_CONTROL_RAM_BLOCKS_RESULT] = "MEMORY_MAP_RESULT",
        [RDMA_CONTROL_COMPRESS] = "COMPRESS",
        [RDMA_CONTROL_REGISTER_REQUEST] = "REGISTRATION_REQUEST",
        [RDMA_CONTROL_REGISTER_RESULT] = "REGISTRATION_RESULT",
        [RDMA_CONTROL_REGISTER_FINISHED] = "REGISTRATION_DONE",
        [RDMA_CONTROL_UNREGISTER_REQUEST] = "UNREGISTRATION_REQUEST",
        [RDMA_CONTROL_UNREGISTER_FINISHED] = "UNREGISTRATION_DONE",
    };
                                         ...
                                         const char *name, bool wr)
{
    ...
    int ret;
    int advice = wr ? IBV_ADVISE_MR_ADVICE_PREFETCH_WRITE : IBV_ADVISE_MR_ADVICE_PREFETCH;
...
{
    return wrid < RDMA_WRID_RECV_CONTROL ? rdma->send_comp_channel : rdma->recv_comp_channel;
}
    ...
    while (test_bit(chunk, block->transit_bitmap))
    {
        ...
        if (ret < 0)
        {
            error_setg(errp, "Failed to Wait for previous write to complete "
                             "block %d chunk %" PRIu64 " current %" PRIu64 " len %" PRIu64 " %d",
...
static int qemu_rdma_accept(RDMAContext *rdma)
{
     ...
     */
    if ((migrate_postcopy() || migrate_return_path()) && !rdma->is_return_path)
    {
...
int rdma_registration_handle(QEMUFile *f)
{
    RDMAControlHeader reg_resp = {
        .len = sizeof(RDMARegisterResult),
        .type = RDMA_CONTROL_REGISTER_RESULT,
        .repeat = 0,
    };
    RDMAControlHeader unreg_resp = {
        .len = 0,
        .type = RDMA_CONTROL_UNREGISTER_FINISHED,
        .repeat = 0,
    };
    ...
    do
    {
        ...
        case RDMA_CONTROL_RAM_BLOCKS_REQUEST:
            ...
            blocks.len = rdma->local_ram_blocks.nb_blocks * sizeof(RDMADestBlock);
...
int rdma_registration_stop(QEMUFile *f, uint64_t flags)
{
    ...
    if (flags == RAM_CONTROL_SETUP)
    {
        ...
        ret = qemu_rdma_exchange_send(rdma, &head, NULL, &resp,
                                      &reg_result_idx, rdma->pin_all ? qemu_rdma_reg_whole_ram_blocks : NULL,
                                      &err);
        if (ret < 0)
        {
            error_report_err(err);
